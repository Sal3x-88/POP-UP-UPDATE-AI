(() => {
  ...
  const SCRIPT_GUARD = "__airbet88_loggedin_popup_v4_loaded__";
  ...
  let popupInitTimer = null;
  let infoTabTimer = null;
  ...
  function canShowPopup() {
    ...
    const lastClosed = Number(
      storageGet(localStorage, DELAY_KEY) || 0
    );
  ...
  function initLoggedInPopup() {
    ...
    // Try immediately, then retry for dynamic/SPA pages.
    tryCreate();

    if (!popupCreated && retries < 40) {
      popupInitTimer =
        window.setInterval(
          tryCreate,
          500
  ...
  function init() {
    ...
    // Helper untuk testing dari browser console.
    window.AIRBET88Popup = {
      resetDelay() {
        storageRemove(
          localStorage,
          DELAY_KEY
